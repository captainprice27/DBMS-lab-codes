...
-- Retrieve the name of the youngest student(s) from the ‘ELE’ department
...
-- according to the dataset 5th highest datincome is not possible 
-- beacuse we have only 2 entries . so it is made OFFSET 1
...
-- so that no 1452 errors are thrown ...

INSERT INTO students
VALUES
    (92005028, 'Sourik Chatterjee', '1995-05-15', 'CSE', 3, 60000.0),
    (92005071, 'Prayas Stokes', '1994-02-20', 'CSE', 3, 55000.0),
    (92005069, 'Jos Buttler', '1993-09-10', 'CSE', 6, 62000.0);
...
-- Create another view of all students in dept 
-- Mechanical Engineering (department Name).
-- The view will contain attributes namely Roll-No, Name, Department Name, Age.

CREATE VIEW mech_stud AS
SELECT rollno, name, hostel , TIMESTAMPDIFF(YEAR, s.bdate, CURDATE()) AS age
FROM students AS  s
WHERE deptcode = 'MEC';
...
VALUES (92005059, 'Soham Banerjee', '1993-05-15', 'MEC', 3, 60000);	

-- Delete a student (for a given Name) of the same department

DELETE FROM students
WHERE name = 'Soham Banerjee' AND deptcode = 'MEC';

-- Shift a student (for a given Name) from Mechanical to Computer Science.

-- DELETE FROM students
-- WHERE name = 'Student Name' AND deptcode = 'MEC';

-- INSERT INTO students (rollno, name, bdate, deptcode, hostel, parent_inc)
-- VALUES (new_rollno, 'Student Name', 'YYYY-MM-DD', 'CSE', new_hostel, new_parent_inc); 
